...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($category == "Fashion" && empty($subcategory)) $subcategoryErr = "Subcategory is required";
    if (empty($_POST["sizes"])) $sizeErr = "At least one size is required";
    else $sizes = $_POST["sizes"];
    ...
    if ($_FILES["product_image"]["error"] == 0) {
        ...
        if (in_array(strtolower($imageType), $allowedTypes) && $imageSize <= $maxSize) {
            $imageNewName = uniqid("product_", true) . "." . $imageType;
            $uploadDir = "uploads/products/";
            if (!is_dir($uploadDir)) mkdir($uploadDir, 0777, true);
            $imagePath = $uploadDir . $imageNewName;
            if (!move_uploaded_file($imageTmp, $imagePath)) $imageErr = "Error uploading image.";
        } else {
    ...
    if (!$productNameErr && !$descriptionErr && !$priceErr && !$categoryErr && !$subcategoryErr && !$sizeErr && !$imageErr) {
        ...
        if ($stmt->execute()) $successMessage = "Product added successfully!";
        else $errorMessage = "Error: Could not add product.";
...
<div class="product-container">
    ...
    <form method="post" enctype="multipart/form-data">
        ...
        <div class="form-group" id="subcategory-group" style="display: none;">
            <label>Subcategory (for Fashion)</label>
            <select name="subcategory">
                <option value="">-- Select Subcategory --</option>
                <option value="Men" <?= ($subcategory == "Men") ? "selected" : "" ?>>Men</option>
                <option value="Women" <?= ($subcategory == "Women") ? "selected" : "" ?>>Women</option>
                <option value="Kids" <?= ($subcategory == "Kids") ? "selected" : "" ?>>Kids</option>
            </select>
