...
$dbname = "Flipkart"; // Your database name
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($phoneErr) && empty($passwordErr)) {
        // Prepare the SQL query to fetch the user data from the database
        ...
        // Check if a user was found with the given phone number
        if ($result->num_rows > 0) {
            ...
            if (password_verify($password, $row["Password"])) {
                // Login successful - redirect to a dashboard or homepage
